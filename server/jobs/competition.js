...
const trackReachedMinEntriesFilter = minimumEntries => {
    return function filter(track) {
        return track.entries >= minimumEntries;
    };
};

const trackReachedMinPlaysFilter = minimumConsumptions => {
    return function filter(track) {
        return (track.wins + track.losses) >= minimumConsumptions;
    };    
};

const trackNotReachedMinPlaysFilter = minimumConsumptions => {
    return function filter(track) {
        return (track.wins + track.losses) < minimumConsumptions;
    };    
};
...
const getMaxEntriesReached = resultTracks => {
    const maxEntriesReached = resultTracks.reduce((acc, track) => {
        return Math.max(acc, track.entries);
    }, -1);
...
const getMostPlays = resultTracks => {
    const maxEntriesPlayed = resultTracks.reduce((acc, track) => {
        return Math.max(acc, (track.wins + track.losses));
    }, -1);
...
const addPreviouslySkippedToTracks = async (tracks, genre) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        previouslySkippedTracksStream.on('end', async function() {
            resolve(count);
        });
        previouslySkippedTracksStream.on('error', async function(err) {
            reject(err);
        });
    });
};
...
module.exports = {
    ...
    finalizeCompetition: async (req, res) => {
        ...
        const competitionResults = genres.map(genre => {
            return processYesterdaysMatchups(genre);
        });
