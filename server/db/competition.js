...
*/
const resolveCurrentStage = competitionState => {
    return competitionState.winner.key && !competitionState.winner.isPlayed ? COMPETITION_STAGES.WINNER: 
...
const queries = {
    ...
    WINNER_DURATION,
    COMPETITION_COLUMNS: COMPETITION_COLUMNS,
    COMPETITION: {
        MIN_DAY_MATCHUPS: 1
    },
    TRACK_KEYS,
    ...
    getMatchupTrackKeys: () => {
        return [ TRACK_KEYS.TRACK_1, TRACK_KEYS.TRACK_2 ];
    },

    getAllTrackKeys: () => {
        return queries.getMatchupTrackKeys().concat([ TRACK_KEYS.WINNER ]);
    },    
    ...
    getMatchupNonPlayingTrack: competitionState => {
      ...
      insertMatchupResults: async (winningTrackId, losingTrackId, winningTrackKey, genre, enteredTrackId) => {
        const insertMatchupQuery = 
            `INSERT INTO matchup (winning_track_id, losing_track_id, winning_track_key, genre, entered_track_id) 
            VALUES (
